package com.brain.revenu.web.rest;
...
import com.brain.revenu.config.KafkaProperties;
...
class RevenuKafkaResourceIT {
    ...
    void setup() {
        ...
        RevenuKafkaResource kafkaResource = new RevenuKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/revenu-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/revenu-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
